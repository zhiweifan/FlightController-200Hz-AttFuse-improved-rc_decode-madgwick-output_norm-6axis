...
void PID_DeInit(PID_TypeDef *pid,float out_abs_max,float proportion_limit_coef,float integration_limit_coef,float dev_limit_coef,float ddev_limit_coef)
{
	...
	if(integration_limit_coef<=0||integration_limit_coef>2)
	{
...
//	return x>0?1:-1;
//}
void PID_Calculate(PID_TypeDef *pid,char use_speed,float speed,char use_error,float error)
{
	...
	{
		pid->ErrDev[0]=(pid->ErrDevTc*pid->ErrDev[0]+(pid->Err[0]-pid->Err[1]))/(1.0f+pid->ErrDevTc);
	}
